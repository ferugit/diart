...
from diart.audio import FilePath
from diart.blocks import BasePipelineConfig, PipelineConfig, OnlineSpeakerDiarization
...
class Optimizer:
    def __init__(
        ...
        self.RhoUpdate_value = hyp_value
    ...
    def __call__(self, num_iter: int, show_progress: bool = True):
        ...
        while tau_del_rho <= 2:
            ...
            self.benchmark.hyp_value = tau_del_rho
            # Update the tau_active hyperparameter value
            self.study.enqueue_trial({
                "tau_active": tau_del_rho
            }, skip_if_exists=True)

            # Optimize for the current hyperparameters
            self.study.optimize(self.objective, n_trials=1, callbacks=[self._callback])

            tau_del_rho += 0.05

            # Check if the tau_active value has reached the stopping condition
            if tau_del_rho > 2:
                break
